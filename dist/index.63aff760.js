...
_viewJs.UI.BUTTON_SEARCH.addEventListener("click", (e)=>{
    e.preventDefault();
    const currentCity = _viewJs.UI.INPUT.value;
    _apiJs.getWeather(currentCity, saveUpdateWeather);
    _viewJs.UI.INPUT.value = "";
    _viewJs.UI.NOW.IMAGE_HEART.classList.remove("image-heart");
});
function updateTabs() {
    const storageData = _storageJs.storage.getCurrentCity();
    _viewJs.UI.NOW.TEMP_INDICATOR.textContent = storageData.temp;
    _viewJs.UI.NOW.IMAGE.src = `http://openweathermap.org/img/wn/${storageData.image}@2x.png`;
    _viewJs.UI.NOW.CITY.textContent = storageData.city;
    _viewJs.UI.DETAILS.CITY.textContent = storageData.city;
    _viewJs.UI.DETAILS.TEMP_INDICATOR.textContent = `Temperature: ${storageData.temp}°`;
    _viewJs.UI.DETAILS.FEELS_LIKE.textContent = `Feels like: ${storageData.feels_like}°`;
    _viewJs.UI.DETAILS.WEATHER.textContent = `Weather: ${storageData.weather}`;
    _viewJs.UI.DETAILS.SUNRISE.textContent = `Sunrise: ${storageData.sunrise}`;
    _viewJs.UI.DETAILS.SUNSET.textContent = `Sunset: ${storageData.sunset}`;
}
updateTabs();
function saveUpdateWeather(weather) {
    _storageJs.storage.saveCurrentCity(new _apiJs.createData(weather));
    updateTabs();
}
...
function fillFavoriteList() {
    const storageList = _storageJs.storage.getFavoriteCities();
    if (storageList) storageList.forEach((city)=>{
        _viewJs.UI.FavoriteList.innerHTML += createTemplate(city);
        _viewJs.UI.NOW.IMAGE_HEART.classList.add("image-heart");
    });
}
fillFavoriteList();
_viewJs.UI.NOW.BUTTON_HEARTH.addEventListener("click", ()=>{
    const storageList = _storageJs.storage.getFavoriteCities();
    const storageData = _storageJs.storage.getCurrentCity();
    const isNoteValidCity = _viewJs.UI.NOW.CITY.textContent === "City";
    const checkImageHeartClass = _viewJs.UI.NOW.IMAGE_HEART.classList.contains("image-heart");
    if (isNoteValidCity || checkImageHeartClass) return false;
    const isNoteValidList = !storageList.includes(storageData.city);
    if (isNoteValidList) _storageJs.storage.saveFavoriteCity(storageData.city);
    else {
        _viewJs.UI.NOW.IMAGE_HEART.classList.add("image-heart");
        return;
    }
    _viewJs.UI.FavoriteList.innerHTML += createTemplate(storageData.city);
    _viewJs.UI.NOW.IMAGE_HEART.classList.add("image-heart");
});
_viewJs.UI.FavoriteList.addEventListener("click", (e)=>{
    const targetCity = e.target;
    const storageList = _storageJs.storage.getFavoriteCities();
    _viewJs.UI.NOW.IMAGE_HEART.classList.add("image-heart");
    if (targetCity.classList.contains("button-close")) {
        _storageJs.storage.saveFavorits(storageList.filter((city)=>city !== targetCity.parentNode.innerText
        ));
        targetCity.parentNode.remove();
        _viewJs.UI.NOW.IMAGE_HEART.classList.remove("image-heart");
    } else _apiJs.getWeather(targetCity.innerText, saveUpdateWeather);
});
if (_viewJs.UI.NOW.CITY.textContent === "City") _viewJs.UI.NOW.IMAGE_HEART.classList.remove("image-heart");

},{"./view.js":"de63B","./storage.js":"kS9sU","./api.js":"eqUwj"}],"de63B":[function(require,module,exports) {
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"eqUwj":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "API", ()=>API
);
parcelHelpers.export(exports, "createData", ()=>createData
);
...
function createData(data) {
    ...
    this.sunrise = _dateFns.format(new Date(data.sys.sunrise * 1000), 'HH:mm');
    this.sunset = _dateFns.format(new Date(data.sys.sunset * 1000), 'HH:mm');
}
function getWeather(cityName, callback) {
    const urlWeater = `${API.NOW}?q=${cityName}&appid=${API.API_KEY}&units=metric`;
    fetch(urlWeater).then((response)=>response.json()
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["kQMTH","adjPd"], "adjPd", "parcelRequire332c")
